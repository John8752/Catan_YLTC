...
Persistent dock emphasis follows the projected interaction. The one-time notice lasts 1.5 seconds in a reserved slot, with no pointer interception or focus change. As of the three-column layout update, this slot is in the right rail on desktop and above the map on compact screens (ADR-0008). Required actions use a pale coral-red surface, soft red border, dark red heading and a single brief brightness cue; incoming trades use a quieter treatment. Reduced-motion users retain the static text without the brightness animation.

## Roll and turn audio (2026-09-08)

Two original Web Audio cues provide a short tumbling click for a confirmed dice roll and a rising chime when the viewer receives a setup, regular-turn or paired-action opportunity. `dice_rolled` projects a public `dice-roll` effect even on seven or without production. Action-attention effects mark turn opportunities with an optional `sound` field; incoming offers and mandatory resolutions do not masquerade as a new turn. Both cues consume `GameView.effects` in an immediate audio lane, independently of resource travel. Dice effects never enter the visual queue.

The client deduplicates dice revisions and turn opportunity IDs, including roll-to-action and setup-settlement-to-road transitions. Initial snapshots, reconnect baselines, stale deliveries and locked/muted audio do not accumulate sounds for later replay. Sounds are synthesized locally without external or proprietary assets. AudioContext is feature-detected and created/resumed from a user gesture, with failures contained so plain-HTTP play and game commands continue. The table has a persistent local mute preference; muting stops active cues. Reduced-motion preferences continue to govern animation separately from sound.

Compact action controls open on entering ordinary or paired actions, including after rolling and resolving seven. Users can still collapse them manually, and selecting a build collapses them to make room for map placement. Unrelated snapshots within the same interaction preserve the manual choice.
