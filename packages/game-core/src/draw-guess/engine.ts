...
export function taskFor(state: DrawGuessState, playerId: string): DrawTask | null {
  ...
  return { id: `${state.id}:${step}:${playerId}`, step, albumIndex, kind: step % 2 ? "drawing" : "text", submitted: state.albums[albumIndex]!.pages.length > step };
}
...
export function executeDrawGuess(state: DrawGuessState, actorId: string | null, command: DrawGuessCommand, hostId?: string): DrawGuessState {
  ...
  if (command.type === "reveal") {
    if (actorId !== hostId) throw new DrawGuessError("ONLY_HOST_CAN_REVEAL", "等待房主揭晓下一页");
