...
it("offers three nouns and three playful phrases per hand across every theme", () => {
  ...
  for (let seed = 0; seed < 100; seed++) {
    ...
    for (const hand of hands) {
      expect(hand.filter((word) => nouns.includes(word))).toHaveLength(3);
      expect(regions.filter((words) => hand.some((word) => (words as readonly string[]).includes(word)))).toHaveLength(1);
