...
import { projectGameForPlayer, type GameHistoryPage, type RoomView } from "@catan/protocol/catan";
import { expect, it, vi } from "vitest";
import { HistoryBuffer } from "./history-buffer.js";
import { RoomSessionChangedError } from "../../room-updates.js";
...
it("an older page racing a new push extends history without replacing dynamic state or effects", async () => {
  ...
  const pending = updates.sync.loadEarlierHistory(session, () => new Promise((done) => { resolve = done; }));
...
it("recovers a reconnect gap to previously read history and rejects a replaced seat's pending page", async () => {
  ...
  expect(updates.sync.hasHistoryGap).toBe(true);
  ...
  await updates.sync.loadEarlierHistory(session, async (_game, before) => { expect(before).toBe(101); return page(50, 100); });
  expect(updates.sync.hasHistoryGap).toBe(false);
