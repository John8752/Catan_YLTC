import type { ResourceType } from "@catan/game-core/catan";
...
export function ResourceCard({
  ...
  readonly variant?: "hand" | "compact" | "flight" | "bank";
  ...
  const cardClassName = cn(
    ...
    variant === "bank" && "h-14 w-9 rounded-md lg:h-20 lg:w-full",
  ...
  const illustration = (
    <svg
      className={cn("resource-card-icon", variant === "bank" ? "h-full w-full max-w-14" : variant === "hand" ? "size-7" : "size-5")}
      viewBox="-22 -22 44 44"
      aria-hidden="true"
    >
  ...
  const content = (
    <>
      ...
      {variant === "bank" ? <span className="absolute inset-x-1 top-1 h-1.5 border-y border-white/30" aria-hidden="true" /> : null}
      {variant === "bank" ? <span className="absolute inset-x-1 top-7 bottom-1 grid place-items-center lg:top-8 lg:bottom-1.5" data-resource-illustration="bank">{illustration}</span> : illustration}
      {variant === "bank" ? null : <span className={cn("mt-auto font-black tracking-wide", variant === "hand" ? "text-[11px] lg:text-xs" : "text-[9px] lg:text-xs")}>
        {presentation.label}
      </span>}
      {count === undefined ? null : (
        <strong data-resource-count="true" className={cn("absolute top-1 right-1 grid min-w-6 place-items-center rounded-md bg-[#154c78] px-1 leading-6 shadow-sm", variant === "bank" ? "top-0.5 right-0.5 h-6 rounded-sm text-base leading-none font-black tabular-nums lg:h-7 lg:min-w-8 lg:text-xl" : variant === "hand" ? "text-base" : "text-xs")}>
          {count}
        </strong>
      )}
  ...
  if (interactive) {
    return (
      <button
        ...
        title={variant === "bank" ? ariaLabel ?? presentation.label : undefined}
  ...
  return (
    <div className={cardClassName} aria-label={ariaLabel} title={variant === "bank" ? ariaLabel ?? presentation.label : undefined} data-resource-target={targetId} data-resource-card={resource}>
      {content}
    </div>
  );
}
