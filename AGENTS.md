...
- Use cases from `tests/e2e/viewport-cases.ts`, not ad hoc substitute sizes. Logical full-canvas baselines are 393×852 and 440×956 CSS px, respectively, with both orientations and DPR 3. Never use physical raster resolution as the CSS viewport or increase UI sizes based on DPR.
- Cover both full-canvas and browser-area cases from the installed Playwright device descriptors. Safari bars reduce available space, especially in landscape. When changing viewport/safe-area behavior, also check resizing with browser bars and safe-area insets; do not assume a full logical display is always the usable web area.
- For game-surface changes that affect the map, HUD, shared typography, safe-area handling, breakpoints, viewport fitting or dock layout, run `pnpm test:e2e:mobile` (also included in `pnpm test:e2e`) and inspect four- and six-player screenshots on both primary models in portrait and landscape, including browser-area cases. Check map/port visibility, two-line port content, number-token occlusion, dock bounds, bank/map disclosures, required actions and document overflow.
- For an isolated dialog, drawer, popover or local panel that does not change the game surface or shared breakpoints, use a focused risk matrix instead of the full phone matrix: at minimum the iPhone 16 portrait browser-area and landscape browser-area cases with the longest supported content or maximum player count. Add one desktop case for interaction behavior. Expand to more devices only when the component has device-specific branches, crosses a breakpoint, changes global overflow/safe-area behavior, or the focused checks expose a regression.
