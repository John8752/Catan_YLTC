import { CatanTestUpdates } from "./sync-test-helpers.js";
import { createBaseGame } from "@catan/game-core/catan";
import { projectGameForPlayer, type GameCommandAck, type RoomView } from "@catan/protocol/catan";
import { afterEach, expect, it, vi } from "vitest";
import { RoomSessionChangedError, ROOM_SNAPSHOT_WAIT_MS } from "../../room-updates.js";
...
it("supplies the last displayed game revision to ACK fallback so recovery can include only unseen live effects", async () => {
  const updates = new CatanTestUpdates(session, vi.fn());
  updates.accept(room(2), session);
  const read = vi.fn(async () => room(3));
  await updates.sync.confirm(ack, session, read, false);
...
it("accepts newer room metadata at the same game revision and rejects duplicate/late snapshots", () => {
  const publish = vi.fn(), updates = new CatanTestUpdates(session, publish);
...
it("handles push-before-ack and ack-before-push without a second HTTP read", async () => {
  vi.useFakeTimers();
  const read = vi.fn(), updates = new CatanTestUpdates(session, vi.fn());
  updates.accept(room(3), session);
  await updates.sync.confirm(ack, session, read, true);
  const waiting = updates.sync.confirm({ ...ack, roomRevision: 4, gameRevision: 4 }, session, read, true);
...
it("reads a complete snapshot when an acknowledged push is missing, but never replays the command", async () => {
  vi.useFakeTimers();
  const read = vi.fn(async () => room(3)), publish = vi.fn(), updates = new CatanTestUpdates(session, publish);
  const waiting = updates.sync.confirm(ack, session, read, true);
  await vi.advanceTimersByTimeAsync(ROOM_SNAPSHOT_WAIT_MS);
  await waiting;
  expect(read).toHaveBeenCalledTimes(1);
  expect(publish).toHaveBeenCalledWith(room(3));
  const offline = new CatanTestUpdates(session, vi.fn());
  await offline.sync.confirm(ack, session, read, false);
...
it("does not regress if a newer push arrives while the fallback HTTP request is in flight", async () => {
  ...
  const publish = vi.fn(), updates = new CatanTestUpdates(session, publish);
  const waiting = updates.sync.confirm(ack, session, () => new Promise((resolve) => { respond = resolve; }), false);
  updates.accept(room(4), session);
  respond(room(3));
  await waiting;
...
it("cancels pending acknowledgements and rejects old-seat data after takeover", async () => {
  vi.useFakeTimers();
  const publish = vi.fn(), updates = new CatanTestUpdates(session, publish);
  const pending = updates.sync.confirm(ack, session, vi.fn(), true);
...
it("accepts a legacy full response and rejects an insufficient fallback instead of unlocking stale state", async () => {
  const updates = new CatanTestUpdates(session, vi.fn());
  await updates.sync.confirm({ commandId: "old", room: room(2) }, session, vi.fn(), true);
  await expect(updates.sync.confirm(ack, session, async () => room(2), false)).rejects.toThrow("等待最新状态");
});
