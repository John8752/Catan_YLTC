...
import type { RoomView, PublicSetupAnalysisView } from "@catan/protocol/catan";
...
export class RoomSetupAnalysis {
  ...
  constructor(private readonly findRoom: (id: string) => CatanRoomRecord | undefined, private aiCommentator: AiCommentator | null,
    private readonly projectRoom: (room: CatanRoomRecord, playerId: string) => RoomView,
    private readonly notify: (room: CatanRoomRecord) => void) {}
  ...
  start(room: CatanRoomRecord): void {
  ...
  private isAwaitingSetupAnalysis(roomId: string, sourceRevision: number, matchId: string | null): boolean {
    ...
    const analysis = room?.publicSetupAnalysis;
