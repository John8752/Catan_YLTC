...
describe("Board", () => {
  it("renders every game hex as an SVG target", () => {
    ...
    expect(markup.match(/data-port-link=/g)).toHaveLength(9);
    expect(markup.match(/data-port-endpoint=/g)).toHaveLength(18);
    expect(markup.match(/data-port-resource-icon=/g)).toHaveLength(9);
    expect(markup.match(/data-port-resource-icon="unknown"/g)).toHaveLength(4);
    expect(markup.match(/data-probability-pips=/g)).toHaveLength(18);
    expect(markup.match(/data-probability-pips="5"/g)).toHaveLength(4);
    expect(markup.match(/data-tile-resource-icon=/g)).toHaveLength(19);
    expect(markup.match(/data-tile-resource-icon="brick"/g)).toHaveLength(3);
    expect(markup.match(/data-tile-resource-icon="lumber"/g)).toHaveLength(4);
    expect(markup.match(/data-tile-resource-icon="wool"/g)).toHaveLength(4);
    expect(markup.match(/data-tile-resource-icon="grain"/g)).toHaveLength(4);
    expect(markup.match(/data-tile-resource-icon="ore"/g)).toHaveLength(3);
    expect(markup.match(/data-tile-resource-icon="desert"/g)).toHaveLength(1);
