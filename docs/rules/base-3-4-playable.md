...
- Only the host may change settings, reroll the map or shuffle the seated order. Every mutation uses the current room revision, is broadcast to all members and is rejected after the game starts.
...
- The lobby offers more player colors than seats. Before the game starts, each member may change only their own color; the server rejects a color already held by another member. A vacated color immediately becomes available again.
- The member list is the authoritative setup and turn order. The host may shuffle that order before starting; the resulting order is broadcast and then passed unchanged into the game state.
...
- Every opponent may independently accept or decline. A player may change their response while the offer remains open. Repeating the response already on the table is an idempotent no-op: it does not advance the game revision or append another public-history entry.
- Instead of accepting or declining, an opponent may keep one counteroffer that replaces their previous response. Counteroffers use the same resource-content rules as the original offer and are public to every seated player. Resubmitting resource-for-resource equivalent terms is the same idempotent no-op.
...
- When an unrelated response makes a submitted trade command stale, the client may retry it once against the latest revision only while the same offer and the command's relevant terms remain unchanged. A changed selected counteroffer always requires explicit confirmation again.
...
- The browser requires a second confirmation before submitting any active development-card play (knight, road building, monopoly or resource choice), in both supported profiles. The sidebar shows only the card name and use button. Resource choice and monopoly selection happen inside the confirmation dialog, which shows the selected resource(s) before submission. Canceling preserves resource selection and submits nothing; a changed game revision dismisses stale confirmation. Victory-point cards remain passive. This is input protection, not a change to server-side eligibility or effects.
