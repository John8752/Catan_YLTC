...
export function validatePage(page: EditablePage, expected: "text" | "drawing", allowEmpty: boolean): EditablePage {
  ...
  return { kind: "drawing", strokes: page.strokes.map((stroke) => ({ color: stroke.color, width: stroke.width, points: stroke.points.map(([x, y]: readonly [number, number]) => [x, y] as const) })) };
}
export function pageIsEmpty(page: EditablePage): boolean { return page.kind === "text" ? !page.text.trim() : page.strokes.length === 0; }
